...
{
    bytes = ALIGN16(bytes);
    ASSERT(mem->maxForwd - mem->posForwd >= bytes, "ERROR: %s", STR("Forward allocator out of memory"));
...
{
    bytes = ALIGN16(bytes);
    ASSERT(mem->currFrame < mem->maxStack, "ERROR: %s", STR("Not within stack frame"));
    ASSERT(mem->maxStack - mem->posStack >= bytes, "ERROR: %s", STR("Stack out of memory"));
